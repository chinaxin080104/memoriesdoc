// Code generated by goctl. DO NOT EDIT.
// goctl 1.7.2
// Source: gouser.proto

package users

import (
	"context"

	"gozero/gouser/types/gouser"

	"github.com/zeromicro/go-zero/zrpc"
	"google.golang.org/grpc"
)

type (
	UserCreateRequest  = gouser.UserCreateRequest
	UserCreateResponse = gouser.UserCreateResponse
	UserInfoRequest    = gouser.UserInfoRequest
	UserInfoResponse   = gouser.UserInfoResponse

	Users interface {
		UserInfo(ctx context.Context, in *UserInfoRequest, opts ...grpc.CallOption) (*UserInfoResponse, error)
		UserCreate(ctx context.Context, in *UserCreateRequest, opts ...grpc.CallOption) (*UserCreateResponse, error)
	}

	defaultUsers struct {
		cli zrpc.Client
	}
)

func NewUsers(cli zrpc.Client) Users {
	return &defaultUsers{
		cli: cli,
	}
}

func (m *defaultUsers) UserInfo(ctx context.Context, in *UserInfoRequest, opts ...grpc.CallOption) (*UserInfoResponse, error) {
	client := gouser.NewUsersClient(m.cli.Conn())
	return client.UserInfo(ctx, in, opts...)
}

func (m *defaultUsers) UserCreate(ctx context.Context, in *UserCreateRequest, opts ...grpc.CallOption) (*UserCreateResponse, error) {
	client := gouser.NewUsersClient(m.cli.Conn())
	return client.UserCreate(ctx, in, opts...)
}
